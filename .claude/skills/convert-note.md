...
2. Scan the target file(s) with Read.
3. Decompose into atomic nodes — one concept per node, 50-300 words each.
4. For each node:
   - Classify with exactly one of the 16 types: `pillar decision concept question playbook task event pattern hypothesis fact source bookmark note contact reference custom`
...
7. Confirm: "Converted X nodes from `raw/filename`. INDEX.md updated."
